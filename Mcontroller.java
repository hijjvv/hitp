		...
		response.setContentType("text/html; charset=euc-kr"); //한글이 인코딩
		   ...
		   try {
			out = response.getWriter();
	           out.println("<script>alert('시간이 지나 로그아웃 되었습니다 로그인을 해 주세요.');  location.href='/jwbook/mcontrol?action=loginl' </script>");
	           out.flush();
	}catch (IOException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
...
//	session.setAttribute("pw", "");
	...
	response.setContentType("text/html; charset=euc-kr"); //한글이 인코딩
	   ...
	   try {
		out = response.getWriter();
        out.println("<script>alert('로그아웃되었습니다.');  location.href='/jwbook/mcontrol?action=main' </script>");
        out.flush();
	} catch (IOException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
